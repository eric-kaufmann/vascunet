...
class VesselAutoencoder(L.LightningModule):
    def __init__(self, learning_rate=1e-2, in_features=2048, out_features=2048, batch_size=2):
        super().__init__()
    ...
    def training_step(self, batch, batch_idx): 
        ...
        loss = torch.norm(pred - sys_vel, dim=1).sum()
...
class VesselModelSinlgePoint2(L.LightningModule):
    def __init__(self, learning_rate=1025, in_features=1024, out_features=3, encoding_features=16, batch_size=2):
        super().__init__()
        self.encoder = nn.Sequential(
            ...
            nn.Linear(64, 32),
            nn.ReLU(),
            nn.Linear(32, encoding_features),
            nn.ReLU(),
        )
        
        self.model = nn.Sequential(
            nn.Linear(encoding_features+3, 16),
            nn.ReLU(),
            nn.Linear(16, 16),
            nn.ReLU(),
            nn.Linear(16, 16),
    ...
    def training_step(self, batch, batch_idx): 
        mesh_points = batch['mesh_tensor']
        fluid_points = batch['fluid_points']
        #print("fluid_points shape", fluid_points.shape)
        sys_vel = batch['sys_vel_tensor']

        vessel_encoding = self.encoder(mesh_points)
        #print("vessel_encoding shape", vessel_encoding.shape)

        input_tensor = torch.concatenate((fluid_points.reshape(self.batch_size, 3), vessel_encoding), dim=1) 
        pred = self.model(input_tensor)
        loss = nn.functional.mse_loss(pred, sys_vel)
        #loss = torch.sum(torch.abs(pred - sys_vel))
        self.log("train_loss", loss, prog_bar=True, on_step=True, on_epoch=True, batch_size=self.batch_size)
